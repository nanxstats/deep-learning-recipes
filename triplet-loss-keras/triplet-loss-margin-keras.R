# Matrix factorization with margin-based triplet loss and Keras ----------------
...
# 1. Define margin-based triplet loss ------------------------------------------
...
build_model <- function(n_user, n_item, n_factor) {
  ...
  loss <- list(embed_user, embed_item_positive, embed_item_negative) %>%
    layer_lambda(loss_margin_triplet, output_shape = c(1))
...
ggsave("triplet-loss-margin-movielens.png", p, width = 9, height = 6)
